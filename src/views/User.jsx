...
function User(props) {
    ...
    return (
      ...
      <div className="usercard">
        <div className="usercard__present">
          <img className="usercard__img" src={user.picture.large} alt={user.name.first + ' imagen'} />
          <h3 className="usercard__dni">{user.id.value}</h3>
        </div>
        <div className="usercard__contents">
          <div className="usercard__header">
            <h2 className="usercard__tittle">{user.name.first} <span>{user.name.last}</span></h2>
            <small className="usercard__subtittle">{user.location.state}</small>
          </div>
          ...
          <small className="usercard__bd usercard__small">{user.dob.date.slice(8,10)}/{user.dob.date.slice(5,7)}-{userId}</small>
        </div>
      </div>
      </>
    )
